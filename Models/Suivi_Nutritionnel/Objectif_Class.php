...
// --- AJOUTER DANS Model/Objectif_Class.php ---
    ...
    public static function verifierEtEnvoyerRappels() {
        $db = Config::getConnexion();
        
        // On cherche les objectifs qui doivent recevoir une notification pile à cette minute
        // et on joint la table users pour récupérer le vrai email de l'utilisateur.
        // Le LEFT JOIN vérifie que l'utilisateur n'a pas encore créé de journal pour aujourd'hui.
        $sql = "SELECT o.*, u.email, u.prenom 
                FROM objectif o
                INNER JOIN users u ON o.id_utilisateur = u.id
                LEFT JOIN journal_alimentaire j ON j.id_utilisateur = u.id AND j.date_journal = CURDATE()
                WHERE o.is_notif_enabled = 1 
                AND o.statut = 'en_cours'
                AND DATE_FORMAT(o.heure_notification, '%H:%i') = DATE_FORMAT(NOW(), '%H:%i')
                AND j.id_journal IS NULL";
        ...
        try {
            ...
            if (count($objectifs) > 0) {
                ...
                foreach ($objectifs as $obj) {
                    $email_destinataire = $obj['email'];
                    $prenom = !empty($obj['prenom']) ? htmlspecialchars($obj['prenom']) : 'Salut';
                    
                    $message = "<h3>{$prenom} !</h3><p>Il est l'heure de remplir ton journal pour ton objectif de <strong>" . htmlspecialchars($obj['type_objectif']) . "</strong>.</p>";
                    
                    if(mail($email_destinataire, $sujet, $message, $headers)) {
                        echo "[Succès] Mail envoyé à " . $email_destinataire . " pour l'objectif #" . $obj['id_objectif'] . "\n";
                    }
                }
            } else {
