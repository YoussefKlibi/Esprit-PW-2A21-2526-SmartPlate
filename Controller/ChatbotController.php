...
// Ensure we don't leak PHP warnings/notices into the JSON response
ini_set('display_errors', '0');
ini_set('display_startup_errors', '0');
error_reporting(E_ALL & ~E_NOTICE & ~E_DEPRECATED & ~E_STRICT);

// Start output buffering so any unexpected output can be cleared before sending JSON
if (!ob_get_level()) ob_start();

header('Content-Type: application/json; charset=utf-8');

function jsonOut(array $payload, int $code = 200): void {
    // Clean any buffered output (warnings, notices, stray whitespace)
