...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // =========================================================================
    const formObjectif = document.querySelector('form[action*="ObjectifController.php?action=add"]');
    if (formObjectif) {
        ...
        btn.addEventListener('click', function() {
            ...
            if (updateBtn) {
                ...
                if (window.JOURNAL_CONTROLLER_URL) {
                    ...
                    updateBtn.formAction = '../../../controller/Suivi_Nutritionnel/JournalController.php?action=update&id=' + encodeURIComponent(id);
                }
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setMealEditMode(isEdit, repasId) {
        ...
        if (isEdit && repasId) {
            const baseUrl = window.REPAS_CONTROLLER_URL || '../../../controller/Suivi_Nutritionnel/RepasController.php';
...
(function(){
    ...
    (function() {
        ...
        function getChatStorageKey() {
            var userId = (typeof window !== 'undefined' && window.SP_CHAT_USER_ID)
                ? String(window.SP_CHAT_USER_ID)
                : 'guest';
            return 'sp_chat_history_v1_user_' + userId;
        }

        function loadHistory() {
            try {
                var raw = localStorage.getItem(getChatStorageKey());
                if (!raw) return null;
        ...
        function saveHistory(history) {
            try {
                localStorage.setItem(getChatStorageKey(), JSON.stringify(history.slice(-40)));
            } catch (e) {}
        }
        ...
        async function ask(question) {
            var url = window.CHATBOT_CONTROLLER_URL || '../../../controller/Suivi_Nutritionnel/ChatbotController.php';
