...
document.addEventListener('DOMContentLoaded', function() {
    ...
    attachValidation(document.getElementById('objectifForm'), function(showError) {
        ...
        // Si la notification est activée, s'assurer qu'une heure est renseignée
        const notifCheckbox = document.getElementById('checkNotif');
        const notifTimeEl = document.getElementById('heure_notification');
        if (notifCheckbox && notifCheckbox.checked) {
            if (!notifTimeEl || notifTimeEl.value === '') showError('heure_notification', 'Veuillez saisir une heure de rappel.');
        }
    });
...
window.openEditForm = function(id, id_user, type, poids, debut, fin, statut, is_notif, heure) {
    ...
    form.action = '../../../controller/Suivi_Nutritionnel/ObjectifController.php?action=update&id=' + id;
    ...
    // Notification fields (si présents)
    const notifEl = document.getElementById('checkNotif');
    const timeEl = document.getElementById('heure_notification');
    const timeGroup = document.getElementById('timeInputGroup');
    try {
        const checked = (String(is_notif) === '1' || String(is_notif).toLowerCase() === 'true');
        if (notifEl) notifEl.checked = checked;
        if (timeEl) timeEl.value = (heure && heure.length >= 4) ? heure : '08:00';
        if (timeGroup) timeGroup.style.display = checked ? 'block' : 'none';
    } catch (e) {
        // ignore
    }
...
window.closeObjectifForm = function() {
    ...
    form.action = '../../../controller/Suivi_Nutritionnel/ObjectifController.php?action=add';
...
function openEditForm(id, id_user, type, poids, debut, fin, statut, is_notif, heure) {
    // Normalisation des valeurs
    is_notif = (typeof is_notif === 'undefined' || is_notif === null) ? 0 : is_notif;
    heure = (typeof heure === 'undefined' || heure === null || heure === '') ? '08:00' : heure;
    window.openEditForm(id, id_user, type, poids, debut, fin, statut, is_notif, heure);
}
...
function openAddForm() {
    ...
    form.action = '../../../controller/Suivi_Nutritionnel/JournalController.php?action=add';
    form.reset();
...
function openJournalEdit(btn) {
    ...
    form.action = '../../../controller/Suivi_Nutritionnel/JournalController.php?action=update&id=' + encodeURIComponent(id);
...
function resetRepasFormToAdd() {
    ...
    if (form) {
        form.action = '../../../controller/Suivi_Nutritionnel/RepasController.php?action=add';
        form.reset();
    }
...
function openRepasEdit(btn) {
    ...
    form.action = '../../../controller/Suivi_Nutritionnel/RepasController.php?action=update&id=' + encodeURIComponent(id);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            fetch(`../../../controller/Suivi_Nutritionnel/ObjectifController.php?action=search&term=${encodeURIComponent(term)}`)
                    ...
                    data.forEach(obj => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td style="padding: 10px; display: flex; gap: 8px; align-items: center;">
                                ...
                                <a href="../../../controller/Suivi_Nutritionnel/ObjectifController.php?action=delete&id=${obj.id_objectif}" 
