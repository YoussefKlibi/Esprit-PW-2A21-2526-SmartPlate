document.addEventListener("DOMContentLoaded", () => {
  ...
  const optionPanierInput = document.getElementById("option_panier");
  ...
  btnAdd.onclick = () => {
    ...
    optionPanierInput.checked = true; // default to checked
  };
  ...
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-edit")) {
      ...
      optionPanierInput.checked = row.cells[6].innerText.trim() === 'Oui';
    ...
    if (e.target.classList.contains("btn-delete")) {
      ...
      if (confirm("Voulez-vous supprimer ce produit ?")) {
        window.location.href =
          "../../controller/Produit/ProduitController.php?action=delete&code=" + code;
      }
    }
  });
  ...
  const submitBtnBoutique = document.getElementById("submitBtnBoutique");
  const TelBError = document.getElementById("TelBError");
  ...
  btnAddBoutique.onclick = () => {
    ...
    clearBoutiqueForm();
    if (submitBtnBoutique) {
      submitBtnBoutique.name = "action";
      submitBtnBoutique.value = "add";
      submitBtnBoutique.innerText = "Ajouter";
    }
  };

  const params = new URLSearchParams(window.location.search);
  if (params.get("error") === "boutique") {
    boutiqueModal.style.display = "flex";
  }
  ...
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-delete-boutique")) {
      ...
      if (confirm("Supprimer cette boutique ?")) {
        window.location.href =
          "../../controller/Produit/BoutiqueController.php?action=delete&codeb=" + code;
      }
    }
  });
...
/* ================== CATEGORIE ===================*/
 ...
 const CategorieBtn = document.getElementById("CategorieBtn");
  ...
  stockForm.addEventListener("submit", (e) => {
    ...
    fetch("../../controller/Produit/StockController.php", {
      method: "POST",
      body: formData
    })
    .then(res => res.text())
  ...
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("delete-stock")) {
      ...
      fetch(`../../controller/Produit/StockController.php?action=delete&Code=${code}&CodeB=${codeB}`)
        .then(() => {
          row.remove();
          stockContent.classList.add("open");
        });
...
// =================== CATEGORIE ============================
// ===== MODIF RECENTE: persistance/etat du toggle categorie =====
  const categorieToggleKey = "categorieOpenAfterAction";

  // fermé par défaut au chargement; ouvert uniquement après une action catégorie
  const shouldOpenAfterAction =
    window.location.hash === "#categorie" ||
    sessionStorage.getItem(categorieToggleKey) === "true";

  if (shouldOpenAfterAction) {
    CategorieContent.classList.add("open");
    sessionStorage.removeItem(categorieToggleKey);
  }
  ...
  CategorieForm.addEventListener("submit", () => {
    sessionStorage.setItem(categorieToggleKey, "true");
  });
  ...
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("edit-categorie")) {
      ...
      // passer le formulaire en mode update
      const actionInput = document.getElementById("categorieAction");
      if (actionInput) {
    ...
    const deleteLink = e.target.closest("a[href*='CategorieController.php?delete=']");
    if (deleteLink) {
      sessionStorage.setItem(categorieToggleKey, "true");
    }
  });
